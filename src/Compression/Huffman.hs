...
{-
    ...
    The first line of the encoded file contains the necessary information to decode it,
    i.e. the number of characters and the frequency of each unique character.

    Following that is a continuous list of bytes representing the Huffman-encoded data.
-}
...
encodeFile path = do
    ...
    let freqs       = frequencies contents
        ...
        newContents = (show . sum $ map snd freqs) ++ " "
                      ++ unwords (map (\(x, y) -> show (ord x) ++ "|" ++ show y) freqs)
...
decodeFile path = do
    ...
    ((read -> charCount):unparsedFreqs) <- words <$> hGetLine handle
    encoded <- BS.hGetContents handle

    let freqs = map readFreq unparsedFreqs
        decoded = decode charCount encoded freqs
...
encode str (huffman -> codes)
    = let encodedStr = concatMap (fromJust . (`Map.lookup` codes)) str
          bytes      = map binStrToWord8 $ splitEvery 8 (encodedStr ++ replicate (8 - length encodedStr `mod` 8) '0')
          byteString = BS.pack bytes                                -- Making length divisible by 8 ^^^
      in byteString
    where
...
decode :: Int -> ByteString -> [(Char, Int)] -> String
decode charCount bs (buildHuff -> huffTree)
    = let bytes = BS.unpack bs
          binaryStr = concatMap word8toBinStr bytes
      in traverseTreeAndDecode binaryStr charCount huffTree huffTree
    where
        ...
        traverseTreeAndDecode :: String -> Int -> Huff -> Huff -> String
        -- Return if all characters have been found
        traverseTreeAndDecode _ 0 _ _ = ""

        -- If leaf node is reached, add character, decrement charCount
        -- and go back to root
        traverseTreeAndDecode str charCount root (Leaf _ c)
            = c : traverseTreeAndDecode str (charCount - 1) root root

        -- Otherwise go left or right depending on code
        traverseTreeAndDecode (x:xs) charCount root (Node _ l r)
            = case x of '0' -> traverseTreeAndDecode xs charCount root l
                        '1' -> traverseTreeAndDecode xs charCount root r

        traverseTreeAndDecode [] _ _ _ = ""